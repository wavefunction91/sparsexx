...
void output_sparse_matrix(std::ostream& out, const SpMatType& A) {
  std::vector<sparsexx::detail::value_type_t<SpMatType>> A_dense(A.m() * A.n(),
                                                                 0.);
...
int main(int argc, char** argv) {
  ...
  if(Arcp_1.rowptr() != Arcp_2.rowptr()) throw std::runtime_error("1-2 Rowptr");
  if(Arcp_1.colind() != Arcp_2.colind()) throw std::runtime_error("1-2 Colind");
  if(Arcp_1.nzval() != Arcp_2.nzval()) throw std::runtime_error("1-2 Nzval");

  if(Arcp_1.rowptr() != Arcp_3.rowptr()) throw std::runtime_error("1-3 Rowptr");
  if(Arcp_1.colind() != Arcp_3.colind()) throw std::runtime_error("1-3 Colind");
  if(Arcp_1.nzval() != Arcp_3.nzval()) throw std::runtime_error("1-3 Nzval");
